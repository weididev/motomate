...
export function EndTripModal({
  ...
  const simulatedDistance = liveOdometer ? Math.round((liveOdometer - activeTrip.startOdometer) * 10) / 10 : 0;

  const [inputMode, setInputMode] = useState<'odo' | 'km'>('km');
  const [odoValue, setOdoValue] = useState('');
  const [kmValue, setKmValue] = useState(simulatedDistance > 0 ? simulatedDistance.toString() : '');
  ...
  const handleSave = () => {
    ...
    const duration = differenceInMinutes(new Date(endTime), new Date(activeTrip.startTime)) - activeTrip.totalPausedMinutes;
    const distance = Math.round((finalEndOdo - activeTrip.startOdometer) * 10) / 10;

    const recordedDistance = activeTrip.laps.reduce((acc, lap) => acc + lap.distance, 0);
    const remainingDistance = Math.max(0, distance - recordedDistance);

    const finalLaps = [...activeTrip.laps];
    if (remainingDistance > 0.1) {
      finalLaps.push({
        id: Math.random().toString(36).substr(2, 9),
        timestamp: new Date().toISOString(),
        odometer: finalEndOdo,
  ...
  const duration = differenceInMinutes(new Date(), new Date(activeTrip.startTime)) - activeTrip.totalPausedMinutes;
    ...
    >
      ...
      >
        ...
        <div className="space-y-6">
          ...
          {activeTrip.laps.length > 0 && (
            <div className="space-y-2 mt-4">
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Recorded Laps</p>
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {activeTrip.laps.map((lap, idx) => (
                  <div key={lap.id} className={cn(
                    "px-4 py-2 rounded-xl border flex flex-col min-w-[100px]",
