...
export function Speedometer({ 
  fuelEfficiency, daysRemaining, serviceDueKm, predictedRange,
  currentFuel, fuelCapacity, bike, isDarkMode, activeTrip,
  startTrip, endTrip, bikeAge
}: SpeedometerProps) {
  const percentage = 75;
  const strokeDasharray = 2 * Math.PI * 80;
  const strokeDashoffset = strokeDasharray - (strokeDasharray * 0.75 * percentage) / 100;
  const odoStr = (bike?.odometer || 0).toFixed(1).padStart(8, '0');
  ...
  return (
    <div className="relative flex flex-col items-center justify-center py-6">
      ...
      <div className="mt-6 flex flex-col items-center">
        <div className={cn("px-6 py-4 rounded-[2rem] border flex items-center gap-4 shadow-xl", isDarkMode ? "bg-black/60 border-white/10 shadow-black/50" : "bg-white border-gray-200 shadow-gray-200")}>
        ...
        {!activeTrip ? (
          <button onClick={startTrip} className="relative group flex items-center justify-center">
            ...
            <div className={cn("relative flex items-center gap-4 px-8 py-4 rounded-full border-2 transition-all active:scale-95", isDarkMode ? "bg-[#1E1E24] border-orange-500/30 hover:border-orange-500 text-white" : "bg-white border-orange-500/20 hover:border-orange-500 text-gray-900 shadow-lg shadow-orange-500/10")}>
              ...
              <div className="text-left"><p className="text-[10px] font-black text-orange-500 uppercase tracking-[0.2em]">Ignition</p><p className="text-sm font-black italic uppercase tracking-tight">Begin Journey</p></div>
            </div>
          </button>
        ) : (
          <button onClick={endTrip} className="relative group flex items-center justify-center">
            ...
            <div className={cn("relative flex items-center gap-4 px-8 py-4 rounded-full border-2 border-red-500/50 transition-all active:scale-95", isDarkMode ? "bg-red-500/10 text-red-500" : "bg-red-50 text-red-600 shadow-lg shadow-red-500/10")}>
              ...
              <div className="text-left"><p className="text-[10px] font-black uppercase tracking-[0.2em]">Active Trip</p><p className="text-sm font-black italic uppercase tracking-tight">End Journey</p></div>
            </div>
          </button>
        )}
      </div>
